...
public class CacheDBHelperImpl extends SQLiteOpenHelper implements CacheDBHelper {
  ...
  public void insertOrUpdateFlextimeDay( FlextimeDay flextimeDay ) {
    if ( isEmpty() || !getCachedDate().equals( flextimeDay ) ) {
      cleanup();
      getReadableDatabase().insert( FlextimeTable.TABLE_NAME, null, FlextimeTable.getContentValues( flextimeDay ) );
    }
    else {
      getReadableDatabase().update( FlextimeTable.TABLE_NAME, FlextimeTable.getContentValues( flextimeDay ),
  ...
  public boolean isEmpty() {
    Cursor c = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectAll(), null, null, null,
        null, null );
    if ( c.getCount() > 0 ) {
      return false;
    }
  ...
  public FlextimeDay getCachedFlextimeDay() {
    ...
    String isHoliday = c.getString( 3 );
    c.close();
    return Factory.getInstance().createFlextimeDay( date, timeFrom, timeTo, getWorkBreaksForFlextimeDay( date ),
        Boolean.valueOf( isHoliday ) );
  }

  @Override
  public List<WorkBreak> getWorkBreaksForFlextimeDay( String date ) {
    List<WorkBreak> workBreaks = new ArrayList<WorkBreak>();
    Cursor c = getReadableDatabase().query( BreakTimeTable.TABLE_NAME, BreakTimeTable.selectTime(),
