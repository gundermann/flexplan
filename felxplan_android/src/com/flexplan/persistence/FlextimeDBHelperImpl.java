...
public class FlextimeDBHelperImpl extends SQLiteOpenHelper implements FlextimeDBHelper {
  ...
  public void insertOrUpdateFlextimeDay( FlextimeDay flextimeDay ) {
    getWritableDatabase().insertOrThrow( FlextimeTable.TABLE_NAME, null, FlextimeTable.getContentValues( flextimeDay ) );
    insertWorkBreaks( flextimeDay );
  }

  @Override
  public List<FlextimeDay> getAllFlextimeDays() {
    Cursor flextimeCursor = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectAll(), null,
        null, null, null, null );
  ...
  private List<FlextimeDay> loadFlextimeDays( Cursor flextimeCursor ) {
    ...
    if ( flextimeCursor.moveToFirst() ) {
      do {
        ...
        String isHoliday = flextimeCursor.getString( 3 );
        flextimeDays.add( Factory.getInstance().createFlextimeDay( date, timeFrom, timeTo,
            getWorkBreaksForFlextimeDay( date ), Boolean.valueOf( isHoliday ) ) );
  ...
  private List<WorkBreak> getWorkBreaksForFlextimeDay( String date ) {
    ...
    Cursor breakCursor = db.query( BreakTimeTable.TABLE_NAME, BreakTimeTable.selectTime(),
        getWhere( BreakTimeTable.DATE, date ), null, null, null, null, null );
  ...
  private FlextimeDay findFlextimeDay( int i, int weekOfYear, int year ) {
    String date = DateHelper.getDateByWeekOfYearAsString( i, weekOfYear, year );
    Cursor c = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectAll(),
        getWhere( FlextimeTable.DATE, date ), null, null, null, null );
  ...
  protected String getWhere( String[] columns, Object[] value ) {
    ...
    for ( int index = 0; index < size; index++ ) {
      sb.append( columns[index] ).append( "='" ).append( value[index] ).append( "'" );
      if ( index + 1 < size ) {
        sb.append( " AND " );
      }
    }
  ...
  public boolean insertWorkBreaks( FlextimeDay currentFlextimeDay ) {
    ...
    for ( WorkBreak workBreak : currentFlextimeDay.getWorkBreaks() ) {
      counter++;
      long result = getWritableDatabase().insert( BreakTimeTable.TABLE_NAME, null,
  ...
  private void cleanWorkBreaks( FlextimeDay flextimeDay ) {
    getWritableDatabase().delete( BreakTimeTable.TABLE_NAME, getWhere( BreakTimeTable.DATE, flextimeDay.getDate() ),
        null );
  }
  ...
  public boolean isDateInDB( String newDate ) {
    return getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectAll(),
        getWhere( FlextimeTable.DATE, newDate ), null, null, null, null ).moveToFirst();
  }

  @Override
  public long getStartTimeOfDay( String newDate ) {
    Cursor c = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectTimeFrom(),
  ...
  public long getEndTimeOfDay( String newDate ) {
    Cursor c = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectTimeTo(),
        getWhere( FlextimeTable.DATE, newDate ), null, null, null, null );
    c.moveToFirst();
    return c.getLong( 0 );
  }
  ...
  public boolean isHoliday( String newDate ) {
    Cursor c = getReadableDatabase().query( FlextimeTable.TABLE_NAME, FlextimeTable.selectIsHoliday(),
        getWhere( FlextimeTable.DATE, newDate ), null, null, null, null );
    c.moveToFirst();
    String isHoliday = c.getString( 0 );
    return Boolean.valueOf( isHoliday );
  }
